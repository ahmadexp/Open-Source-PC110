...
| **ES488** | **U4** — ESS **ES488F** "AudioDrive" audio subsystem | Sound Blaster–compatible ISA codec plus the discrete YM3812 (OPL2) FM chain and speaker amp. Now includes the **complete traced audio signal path** — sound is mixed **twice** (inside the ES488, then again in a discrete NJM3414A summing mixer that sets the PCM:FM balance in *hardware*), the YM3014B serial DAC, the DS1669 push-button volume pot, the LM4861 BTL amp and headphone-mute logic — plus **live chip ID** (self-IDs as ES488, running only as **SB 2.0** — mono, 8-bit, no mixer) and two replacement assessments: an ES1488 swap (**not worth it**) and putting an **RP2350 in the U4 socket** to inherit the ISA interface, DMA channel and tuned analogue back-end while presenting SB Pro/SB16 in firmware (**the recommended path**). |
