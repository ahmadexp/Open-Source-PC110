...
### 3.5 Switching regulator (MAX786) + ESR-critical caps — bench-validated sourcing  ✅ **[C, bench-tested]**

The PSU's main rails are generated by **U4 = `MAX786CAI`** (Maxim triple-output notebook power-supply
controller: two synchronous **PWM step-down** channels — the "3" = 3.3 V and "5" = 5 V outputs, with
`DH*/LX*/DL*/CS*/FB*` per channel — driving external FETs + an inductor, plus on-chip linear-regulator
outputs). Its switching stages are the "switched/DC-DC power nodes" on connector pins **12/13/14** (§3.4):
FETs **Q8/Q10**, inductor **L15**, and the bulk output caps **C45/C50/C57/C67**.

**⚠️ ESR matters, not just capacitance.** The MAX786 is sensitive to output-cap **ESR/impedance**: too-high
ESR trips its shutdown (the unit **won't boot** — the MAX cuts off), and insufficient/mis-specified caps
leave the rails noisy (observed as **VGA-output jitter**). When rebuilding, match the **ESR class**, not
only the µF value — most of these are 20 % parts, so exact capacitance is secondary to ESR.

**Bench-validated cap list** (a contributor A/B-tested this across two PC110 units; IBM original ref ↔ this
project's KiCad ref ↔ value ↔ a known-good Mouser part):

| IBM orig | KiCad ref | Value | Mouser part | Notes |
|---|---|---|---|---|
| EC1 | `C50` | 56 µF 25 V | *(TBD)* | lay-down electrolytic, easy to source |
| EC2 | `C45` | 33 µF 20 V | `80-A758EK336M1EAAE40` | **wrong ESR here → MAX786 shuts down, no boot** |
| EC30 | `C54` | 47 µF 6.3 V | `667-35SEK47M` | |
| EC50 | `C56` | 47 µF 6.3 V | `667-35SEK47M` | **insufficient here → slight VGA jitter** |
| EC51 | `C61` | 47 µF 6.3 V | `667-35SEK47M` | **insufficient here → slight VGA jitter** |
| EC70 | `C57` | 15 µF 16 V | `710-870575773001` | exact 15 µF that physically fits is hard to find |
| EC80 | `C67` | 18 µF 50 V | `80-A759KK186M1KAAE75` | slightly larger than the 6.3 mm pad but fits fine |

(Cap values confirmed against `PCB/PSU/Fab/PSU.kicad_pcb_bom.csv`; `U4 = MAX786CAI` confirmed in
`PCB/PSU/PSU.kicad_sch`. Sourcing/failure-mode data is a community bench contribution.)
